...
abc = wd.find_elements("xpath", '//*[@class="google-social-option"]')
...
key=wd.find_element("id", 'googlesearchinput')
key.send_keys(keyword) 

button=wd.find_element(By.CLASS_NAME, 'main-search')
button.click()

get_social_media=wd.find_elements("xpath", '//iframe[@class="cse-iframe"]')
...
for i in links:
	...
	if "facebookcse" in i:
		j=1
		while(j<=10):
			posts=wd.find_elements("xpath", '//div[@class="gsc-webResult gsc-result"]/div/div/div/a')
			...
			for each_link in links_for_post:
				...
				if hasXpath('//div[@class="_5pbx userContent _3576"]'):
					content=wd1.find_element("xpath", '//div[@class="_5pbx userContent _3576"]')
					post_of_user=content.text
					arr_post.append(post_of_user)
			wd1.close()
			j=j+1
			page_numbers=wd.find_elements("xpath", '//div[@class="gsc-cursor-page"]')
		...
		while(j<=10):
			...
			for each_link in links_for_post:
				...
				if hasXpath('//div[@class="js-tweet-text-container"]'):
					content=wd1.find_element("xpath", '//div[@class="js-tweet-text-container"]')
					post_of_user=content.text
					arr_post.append(post_of_user)
			wd1.close()
			j=j+1
		...
		while(j<=10):
			...
			for each_link in links_for_post:
				...
				if hasXpath('//div[@class="C4VMK"]'):
					content=wd1.find_element("xpath", '//div[@class="C4VMK"]')
					post_of_user=content.text
					arr_post.append(post_of_user)
			wd1.close()
			j=j+1
	...
	#For links of Linkedin
		...
		while(j<=10):
			...
			for each_link in links_for_post:
				...
				sleep(3)
				if hasXpath('//p'):
					pars=wd1.find_elements("xpath", '//p')
		...
		while(j<=10):
			...
			for each_link in links_for_post:
				...
				if hasXpath('//button[@class="RCK Hsu mix Vxj aZc GmH adn a_A gpV hNT iyn BG7 NTm KhY"]'):
					butt=wd1.find_element("xpath", '//button[@class="RCK Hsu mix Vxj aZc GmH adn a_A gpV hNT iyn BG7 NTm KhY"]')
					butt.click()
